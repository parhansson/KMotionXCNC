...
export class BufferReader {
  ...
  byte() {
    const val = this.dv.getUint8(this.off)
    this.off += 1
    return val
  }
  short() {
    const val = this.dv.getUint16(this.off, this.littleEndian)
    this.off += 2
    return val
  }
  int() {
    const val = this.dv.getUint32(this.off, this.littleEndian)
    this.off += 4
    return val
  }
  double() {
    const val = this.dv.getFloat64(this.off, this.littleEndian)
    this.off += 8
    return val
  }
  string(): string {
    const len = this.int()
    const val = this.readUtf8String(new Uint8Array(this.dv.buffer), this.off, len)
