...
export class ThreeViewComponent implements AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    let center = new THREE.Vector3(150, 100, 0)
    ...
    //Logic
    let cursor = new THREE.Mesh(new THREE.SphereGeometry(1), new THREE.MeshBasicMaterial({ color: 0xff0000 }));
    ...
    [-1, 0, 0, 0xFFCCCC]].forEach((position) => {
      let light = new THREE.DirectionalLight(position[3]);
...
*/
  ...
  onMouseMove(event) {
    ...
    let intersectionChanged = this.modelDetector.detect(mouseVector) || this.machineDetector.detect(mouseVector);
